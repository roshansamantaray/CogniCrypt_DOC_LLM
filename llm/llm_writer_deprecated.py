...
#!/usr/bin/env python3
# """
# CrySL explanation generator.
# Modes:
# 1. Plain: uses only the CrySL rule content.
# 2. RAG:   augments with retrieved semantic snippets from a CrySL paper PDF.
# """
#
# import os
# import sys
...
# from openai import OpenAI, OpenAIError
#
# # Ensure project root on path (in case executed as a script)
# project_root = Path(__file__).resolve().parent.parent
# if str(project_root) not in sys.path:
#     sys.path.insert(0, str(project_root))
#
# from llm.rag_crysl import (  # noqa: E402
#     build_pdf_index,
#     retrieve_chunks,
#     build_prompt as rag_build_prompt,
#     generate_explanation as rag_generate_explanation,
# )
#
# SECTION_KEYS = ["SPEC", "OBJECTS", "EVENTS", "ORDER",
#                 "CONSTRAINTS", "REQUIRES", "ENSURES",
#                 "FORBIDDEN", "NEGATES"]
#
#
# def _parse_crysl(text: str) -> dict:
#     """
#     Parse a raw CrySL rule (section headers in SECTION_KEYS) into a dict
#     where each section is a single newline-joined string.
#     """
#     pat = re.compile(r"\b(" + "|".join(SECTION_KEYS) + r")\b")
...
#         lines = [l.rstrip() for l in text[start:end].strip().splitlines() if l.strip()]
#         out[key] = "\n".join(lines)
#     return out
#
#
...
#     """
#     Load a rule from a JSON or CrySL text file.
#     JSON expected keys:
#       className, objects, events, order, constraints, requires, ensures, forbidden, negates, explanationLanguage
#     """
#     raw = Path(path).read_text(encoding="utf-8")
#     if raw.lstrip().startswith("{"):
#         obj = json.loads(raw)
#         return {
#             "SPEC": obj.get("className", ""),
#             "OBJECTS": _join_list(obj.get("objects", [])),
#             "EVENTS": _join_list(obj.get("events", "")),
#             "ORDER": _join_list(obj.get("order", "")),
#             "CONSTRAINTS": _join_list(obj.get("constraints", [])),
#             "REQUIRES": _join_list(obj.get("requires", [])),
#             "ENSURES": _join_list(obj.get("ensures", [])),
#             "FORBIDDEN": _join_list(obj.get("forbidden", [])) or "N/A",
#             "NEGATES": _join_list(obj.get("negates", [])) or "N/A",
#             "LANGUAGE": obj.get("explanationLanguage", "English"),
#         }
#     return _parse_crysl(raw)
#
#
# def ensure_defaults(rule: dict, language: str) -> dict:
#     defaults = {
#         "SPEC": "",
#         "OBJECTS": "None",
#         "EVENTS": "N/A",
...
#         rule.setdefault(k, v)
#         if not str(rule[k]).strip():
#             rule[k] = v
#     return rule
#
#
# def build_plain_prompt(rule: dict) -> str:
#     return f"""
# You are a CrySL explanation assistant (no external evidence; only rule content).
#
# Rule Sections:
# SPEC: {rule['SPEC']}
# OBJECTS:
...
#
# Instructions:
# 1. Use ONLY provided sections; no external facts.
# 2. If a section is 'N/A' or 'None', state it is unspecified.
# 3. Sections: Overview; Correct Usage; Parameters & Constraints; Predicates; Forbidden Patterns; Security Rationale.
# 4. No invented APIs or constraints.
# Respond in {rule['LANGUAGE']}.
# """.strip()
#
#
...
#         model=model,
#         messages=[
#             {"role": "system", "content": "You produce accurate, self-contained CrySL explanations."},
#             {"role": "user", "content": prompt},
#         ],
#         temperature=0.0,
#         max_tokens=1300,
...
#
# def main():
#     parser = argparse.ArgumentParser(description="CrySL explanation generator (plain or PDF RAG)")
#     parser.add_argument("rule_path")
#     parser.add_argument("language", nargs="+", help="Target natural language (first token used)")
#     parser.add_argument("--model", default="gpt-4o-mini")
#     parser.add_argument("--paper-pdf", help="Path to CrySL paper PDF for RAG")
#     parser.add_argument("--emb-model", default="text-embedding-3-small")
#     parser.add_argument("-k", type=int, default=6, help="Top-k chunks to retrieve in RAG mode")
#     parser.add_argument("--cache-dir", default="rag_cache", help="Cache directory for PDF embeddings")
#     args = parser.parse_args()
#
...
#     rule = ensure_defaults(rule, language)
#
#     if args.paper_pdf:
#         idx, chunks = build_pdf_index(
#             args.paper_pdf,
...
#             emb_model=args.emb_model,
#         )
#         prompt = rag_build_prompt(rule, retrieved, language)
#         try:
#             out = rag_generate_explanation(prompt, model=args.model)
#         except OpenAIError as e:
#             raise SystemExit(f"LLM error: {e}")
#         print(clean_output(out))
#     else:
#         client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
#         prompt = build_plain_prompt(rule)
#         try:
#             out = call_llm(client, args.model, prompt)
#         except OpenAIError as e:
#             raise SystemExit(f"LLM error: {e}")
#         print(clean_output(out))
#
